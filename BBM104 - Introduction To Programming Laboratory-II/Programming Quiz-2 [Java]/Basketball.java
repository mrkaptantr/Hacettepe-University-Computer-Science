...
public class Basketball extends Sports{
	...
	public static void calculateResults(List<String> typeOfSport) {
		...
		for (int i=0; i<typeOfSport.size(); i++) {
			if (typeOfSport.get(i).compareTo("B") == 0){
				players.add(i);
			}
		}
	...
	public static void writeFiles(){
		for (int i=0; i<clubNames.size(); i++) {
			printFile.add(Integer.toString(clubNames.size() -i) + ".\t" + clubNames.get(i) + "\t" + (win[i] + lose[i]) + "\t" + win[i] + "\t" + tie[i] + "\t"+ lose[i] + "\t" + sets2[i] + "\t" + scores[i] + "\n");
		}		
		FileOperations.writeOutput("basketball", printFile);
	}
}
