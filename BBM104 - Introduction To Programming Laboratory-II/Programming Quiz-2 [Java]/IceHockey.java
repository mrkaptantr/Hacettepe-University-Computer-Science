...
public class IceHockey extends Sports{
	...
	public static void calculateResults(List<String> typeOfSport) {
		...
		for (int i=0; i<typeOfSport.size(); i++) {
			if (typeOfSport.get(i).compareTo("I") == 0){
				players.add(i);
			}
		}
		...
		for (int j=0; j<players.size(); j++) {
			if (Integer.parseInt(result.get(players.get(j)).split(":")[0]) > Integer.parseInt(result.get(players.get(j)).split(":")[1])) {					int index = (clubNames.indexOf(clubs.get(players.get(j))));
			...
			if (Integer.parseInt(result.get(players.get(j)).split(":")[0]) < Integer.parseInt(result.get(players.get(j)).split(":")[1])) {
			...
			if (Integer.parseInt(result.get(players.get(j)).split(":")[0]) == Integer.parseInt(result.get(players.get(j)).split(":")[1])) {
				int index1 = (clubNames.indexOf(clubs.get(players.get(j))));
				scores[index1] = scores[index1] + 1;
				tie[index1] = tie[index1] + 1;
				sets[index1 * 2] = sets[index1 * 2] + Integer.parseInt(result.get(players.get(j)).split(":")[0]);
				sets[(index1 * 2) + 1] = sets[(index1 * 2) + 1] + Integer.parseInt(result.get(players.get(j)).split(":")[1]);
				int	index2 = (clubNames.indexOf(clubs2.get(players.get(j))));
				scores[index2] = scores[index2] + 1;
				tie[index2] = tie[index2] + 1;
				sets[index2 * 2] = sets[index2 * 2] + Integer.parseInt(result.get(players.get(j)).split(":")[1]);
				sets[(index2 * 2) + 1] = sets[(index2 * 2) + 1] + Integer.parseInt(result.get(players.get(j)).split(":")[0]);
			}		
		}
	...
	public static void writeFiles(){
		...
		FileOperations.writeOutput("icehockey", printFile);	
	}
}